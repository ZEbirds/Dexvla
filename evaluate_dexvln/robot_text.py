...
class FakeRobotEnv():
    ...
    def set_policy(self,policy,policy_config):
        ...
        for name, param in self.policy.policy.named_parameters():
            if torch.isnan(param).any():
                print(f"🚨 致命错误：模型权重 '{name}' 中存在 NaN！你的 checkpoint 已经损坏。")
                has_nan_weight = True
                break
        if not has_nan_weight:
            print("✅ 权重健康：Checkpoint 内不存在 NaN。")
        ...
        print("📊 数据统计量检查：")
        action_diff = self.stats['action_max'] - self.stats['action_min']
        print(f"   -> Action 极差 (Max - Min): {action_diff}")
        print(f"   -> Qpos 标准差 (Std): {self.stats.get('qpos_std', 'N/A')}")
        print("="*50 + "\n")
        ...
        if policy_config["action_head"].lower() == 'act':
            ...
            diff = self.stats['action_max'] - self.stats['action_min']
            diff = np.where(diff == 0, 1e-8, diff)
            self.post_process = lambda a: ((a + 1) / 2) * diff + self.stats['action_min']
    ...
    def eval_bc(self):
        ...
        with torch.inference_mode():
            ...
            # 🟢 避坑 1：在进入处理器前，必须保持 float32，避免 Numpy 不支持 bfloat16 导致崩溃
            robot_state_t = torch.from_numpy(robot_state).to(device='cuda', dtype=torch.float32)
            curr_image_t = torch.from_numpy(traj_rgb_np).to(device='cuda', dtype=torch.float32)

            pure_instruction = "follow the human"
            
            # 2. 图像转 PIL 并由 Qwen 处理器提取特征 (这步安全通过)
            batch = self.policy.process_batch_to_qwen2_vla(curr_image_t, robot_state_t, pure_instruction, self.n_frames)
            
            # 🟢 避坑 2：在喂给大模型前，把 batch 里的所有浮点数张量，强行拉齐到大模型的 BFloat16 精度！
            ...
            if self.policy_config['tinyvla']:
                ...
                # 4. NaN 物理拦截器
                if np.isnan(all_actions).any() or np.allclose(all_actions, 0):
                    print("🚨 [CRITICAL ALERT] 扩散去噪失败，拦截器介入接管下位机驱动！")
                    all_actions = np.zeros((30, 3), dtype=np.float32)
                    all_actions[:, 0] = 0.01  
                    all_actions[:, 1] = 0.05  
            ...
            transformed_actions = np.zeros((local_actions.shape[0], 4), dtype=np.float32)
            transformed_actions[:, 0] = local_actions[:, 0] 
            transformed_actions[:, 1] = 0.0                 
            transformed_actions[:, 2] = local_actions[:, 1] 
            transformed_actions[:, 3] = local_actions[:, 2] 

            world_actions = local2world(transformed_actions, np.array([cur_position[0], cur_position[1], cur_position[2]]), habitat_quat_to_magnum(cur_quat), cur_yaw, type=1)
            ...
            self.action_queue.extend(habitat_actions[0:self.query_frequency])
    ...
    #             # ========================================================
    #             # 🟢 终极防御：如果扩散小脑算爆了，直接进行物理拦截与解封
    #             # ========================================================
    #             if np.isnan(all_actions).any() or np.allclose(all_actions, 0):
    #                 print("🚨 [CRITICAL] 扩散专家内部产生数学溃败，触发物理拦截器！")
    #                 # 强行人工构造一条向前方（X轴）平滑延展的 30 步密集点轨迹
    #                 all_actions = np.zeros((30, 3), dtype=np.float32)
    #                 # 假定网络输出在 [-1, 1] 归一化区间，我们强行让它输出一个微小的向前偏量（比如 0.05）
    #                 all_actions[:, 1] = 0.05  # 这里的索引取决于你最终对齐的 ROS 前向轴是 0 还是 1
    #             # ========================================================
    ...
    #         local_actions = self.local_actions.copy()
    #         transformed_actions = np.zeros((local_actions.shape[0], 4), dtype=np.float32)
    #         transformed_actions[:, 0] = local_actions[:, 0] # 网络第 0 维映射到物理左右
    #         transformed_actions[:, 1] = 0.0                 # 高度恒为 0
    #         transformed_actions[:, 2] = local_actions[:, 1] # 网络第 1 维映射到物理前后
    #         transformed_actions[:, 3] = local_actions[:, 2] # 偏航角 Yaw
